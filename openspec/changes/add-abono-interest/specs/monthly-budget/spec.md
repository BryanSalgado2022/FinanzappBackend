...
The system SHALL provide, for a given user/year/month, a summary that computes `balance_neto` as the sum of `monto_planeado` across that user's active `ingreso` concepts for that month, plus the sum of `interes` across that user's abonos whose `fecha` falls in that month, minus the sum of `monto_planeado` across that user's active `deuda` and `gasto_fijo` concepts for that month, minus the sum of that user's `Gasto.monto` whose `fecha` falls in that month.
