...
def list_entries(
    ...
    try:
        concept_service.get_concepto(session, current_user.id, concepto_id)
    ...
    return list(entry_service.list_entries(session, concepto_id))
...
def upsert_entry(
    ...
    return entry_service.upsert_monthly_entry(
        session,
        concepto,
        anio,
        mes,
