...
- Cualquier concepto (`deuda`, `gasto_fijo` o `ingreso`) puede llevar `dia_vencimiento` (1-28, opcional) — a diferencia de otros campos, es **siempre editable** porque es puramente informativo y no dispara ningún recálculo. Cada entrada mensual expone `vencida` (calculado al vuelo: no pagada y con fecha de vencimiento ya pasada).
- `EntradaMensual.fecha_pago`, `Concepto.finalizado_en` y `Deudor.finalizado_en` (todos nullable) se asignan **automáticamente por el backend, nunca por el cliente** — `fecha_pago` se pone en `date.today()` justo cuando `pagado` pasa de `false` a `true` (y se limpia si vuelve a `false`); `finalizado_en` se pone en `date.today()` justo cuando `activo` pasa de `true` a `false` (y se limpia si se reactiva). Ambos comparan el valor ANTERIOR contra el nuevo antes de sobreescribir — reguardar una entrada/concepto ya pagado/activo sin cambiar ese estado no pisa la fecha existente. Pensado para que la futura Agenda de pagos pueda ubicar "se pagó esto" o "se terminó esto" en un día exacto del calendario.
...
- `User.color_acento` (opcional, `null` = usar el color por defecto de la app) es un identificador de un set curado de 9 (`ALLOWED_ACCENT_COLORS` en `app/models/user.py`: verde, azul, morado, rosa, naranja, amarillo, rojo, turquesa, gris) — el backend NO conoce valores de color/CSS, solo valida y guarda el identificador; el frontend decide a qué hex/tema corresponde cada uno. En `PATCH /users/me`, a diferencia de `categoria_ids` (donde `[]` vacío es la señal de "limpiar"), aquí el campo es un único `str | None`: como tanto "omitido" como "`color_acento: null`" deserializan al mismo `None` en Pydantic, el router usa `payload.model_fields_set` (no el valor deserializado) para distinguir "no tocar" de "limpiar explícitamente al default" — un chequeo `is not None` habría sido insuficiente.
